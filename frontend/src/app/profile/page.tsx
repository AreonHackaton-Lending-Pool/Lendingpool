...
export default function Profile() {
    ...
    // console.log(withDrawData, withdrawError)
    ...
    const { data: setCollateralAmountData, isLoading: setCollateralAmountIsLoading, isSuccess: setCollateralAmountIsSuccess, error: setCollateralAmountError, status: setCollateralAmountStatus } = useSimulateContract({
        ...
        args: [setCollateralIndex as number],
    })
    ...
    const { data: cancelCollateralData, isLoading: cancelCollateralIsLoading, isSuccess: cancelCollateralIsSuccess, error: cancelCollateralError, status: cancelCollateralStatus } = useSimulateContract({
        ...
        args: [cancelCollateralIndex as number],
        account: account.address as `0x${string}`,
    })

    // console.log(cancelCollateralData, cancelCollateralError)
    ...
    return (
        <>
            ...
            <div>
                ...
                <button disabled={simulatedContractError || amount < 0.005} onClick={() => writeContract(simulatedContractData?.request)} className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded disabled:bg-gray-400">
            ...
            <div>
                ...
                <button onClick={() => writeContract(withDrawData?.request as any, Number(index) as any)} className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded disabled:bg-gray-400 w-7">Withdraw</button>
            </div>
            ...
            <div>
                ...
                <button onClick={() => writeContract(getLoanData?.request, Number(loanAmount))} className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded w-7">Get Loan</button>
            ...
            <div>
                ...
                <button onClick={() => writeContract(repayLoanData?.request, Number(repayAmount))} className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">Repay Loan</button>
            </div>
            {/* <Account /> */}
            {/* <Balance /> */}
            {/* <Reading /> */}
            ...
            <input type="number" value={cancelCollateralIndex} onChange={(event) => setCancelCollateralIndex(event.target.value)} className="border border-gray-300 rounded px-2 py-1" placeholder="Collateral Amount (in ETH)" />

            <button onClick={() => writeContract(cancelCollateralData?.request)}>Cancel Collateral</button>

            <input type="number" value={setCollateralIndex} onChange={(event) => setSetCollateralIndex(event.target.value)} className="border border-gray-300 rounded px-2 py-1" placeholder="Collateral Amount (in ETH)" />

            <button onClick={() => writeContract(setCollateralAmountData?.request)}>Set Collateral</button>
            ...
            <div>
                ...
                <div className="flex flex-col mx-6 mt-[-20px] gap-5 xl:flex-row">
                    <div className=" flex-1 flex flex-col gap-3">
                        ...
                        <div className="bg-[#787878]">
                            ...
                            <div className="flex gap-6">
                                ...
                                <h5 className="border p-1">Collateral</h5>
                            </div>
                                        ...
                                        <td className="flex flex-col gap-1 my-2">
                                            <div>
                                                <div className="w-50 flex mb-2">
                                                    <input type="number" onChange={(event) => setAmount(event.target.value)} className="border border-gray-300 rounded px-2 py-1" value={amount} step="any" min="0.005" placeholder="Deposit Amount (in ETH)" />
                                                    <button disabled={simulatedContractError || amount < 0.005} onClick={() => writeContract(simulatedContractData?.request)} className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-6 rounded disabled:bg-gray-400">
                                                        {simulatedContractIsLoading ? 'Loading...' : simulatedContractIsSuccess ? 'Deposit' : 'Error'}
                                                    </button>
                                                </div>
                                                <div className="w-50 flex">
                                                    ...
                                                    <button onClick={() => writeContract(withDrawData?.request as any, Number(index) as any)} className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded disabled:bg-gray-400">Withdraw</button>
                                                </div>
                                            </div>
                            ...
                            <table className="w-full text-center">
                                ...
                                <tbody>
                                    <tr>
                                        ...
                                        <td className="flex gap-1"><button className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">Deposit</button></td>
                                    </tr>
                                    <tr>
                                        <td>WBTC</td>
                                        <td>0.0016</td>
                                ...
                                <tbody>
                                    <tr>
                                        ...
                                        <td className="flex flex-col gap-1 my-2">
                                            <div>
                                                <div className="w-50 flex mb-2">
                                                    <input type="number" onChange={(event) => setLoanAmount(event.target.value)} className="border border-gray-300 rounded py-1" placeholder="Loan Amount (in gwei)" />
                                                    <button onClick={() => writeContract(getLoanData?.request, Number(loanAmount))} className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-2 rounded">Borrow</button>
                                                </div>
                                                <div className="w-50 flex">
                                                    ...
                                                    <button onClick={() => writeContract(repayLoanData?.request, Number(repayAmount))} className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">Repay</button>
                                                </div>
                                            </div>
